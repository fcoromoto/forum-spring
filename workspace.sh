...
function up() {
  if [ "$service" == "" ]; then
    ...
    docker compose -f "$dockerComposeFile" up -d --build  "$service"
  fi
}
...
function down() {
  docker compose -f "$dockerComposeFile" down --remove-orphans --rmi local
}
...
function terminal() {
  validateIfServiceWasInformed
  validateIfServiceExists
  docker compose -f "$dockerComposeFile" exec -ti "$service" /bin/sh
}
...
function start() {
  if [ "$service" == "" ]; then
    ...
    docker compose -f "$dockerComposeFile" start "$service"
  fi
}
...
function validateIfServiceExists() {
  serviceSearched="$(docker compose -f "$dockerComposeFile" ps --format json | jq '.[] | select(.Name=='\"$service\"')')"
